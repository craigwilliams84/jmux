...
/**
 * Build the agent launch prompt for an issue. Tracker-agnostic: the only
 * tracker-specific bit is the label in the opening line (e.g. "Linear", "Jira").
 * The XML-ish structure works for any tracker that maps onto the `Issue` shape.
 */
export function buildIssuePrompt(issue: Issue, trackerLabel: string): string {
  const out: string[] = [];
  out.push(`Work on ${trackerLabel} issue ${issue.identifier}:`);
  out.push("");
